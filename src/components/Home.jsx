...
import { addErrorMessageAction, fetchPost, hasErrorTrueAction, isLoadingFalseAction } from "../redux/action";
...
const Home = () => {
  ...
  return isLoading ? (
    ...
    <Container fluid='lg'>
      <Row>
        <Col xs={2}></Col>
        <Col xs={7}>{posts.map((elem, i) => i < 5 && <SinglePost elem={elem} key={`post${i}`} />)}</Col>
        <Col xs={3}>
          <NewsSidebar />
        </Col>
      </Row>
